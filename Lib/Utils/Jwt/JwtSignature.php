...
final class JwtSignature {
    ...
    public static function generate(string $payload, string $key, JwtAlgorithmType $alg): string {
        switch ($alg->value()) {
            ...
            case 'RS256':
                return self::generate_rsa_signature($payload, $key, OPENSSL_ALGO_SHA256);
            case 'RS384':
                return self::generate_rsa_signature($payload, $key, OPENSSL_ALGO_SHA384);
            case 'RS512':
                return self::generate_rsa_signature($payload, $key, OPENSSL_ALGO_SHA512);
        }
     ...
     * @throws JwtGenerateSignatureError
     */
    private static function generate_rsa_signature(string $source, string $key, int $alg): string {
